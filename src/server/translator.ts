...
function assTimeToMs(assTime: string): number {
...
/** Build a minimal ASS document from translated cues */
function buildAssDocument(
  cues: Array<{ text: string; start: string; end: string }>
): string {
  const lines = [
    "[Script Info]",
    "; Generated by SubSmelt",
    "ScriptType: v4.00+",
    "PlayResX: 1920",
    "PlayResY: 1080",
    "WrapStyle: 0",
    ...
    ...cues.map(
      (c) => `Dialogue: 0,${c.start},${c.end},Default,,0,0,0,,${c.text}`
    ),
  ];
  return lines.join("\n") + "\n";
}
...
export function saveTranslated(
  ...
  const outExt = outputExtension || inputExtension;
  let newSubtitle: string;

  if (["srt", "vtt"].includes(inputExtension) && ["srt", "vtt"].includes(outExt)) {
    // SRT/VTT → SRT/VTT
    const format = outExt === "vtt" ? "WebVTT" : "SRT";
  ...
  } else if (["ass", "ssa"].includes(inputExtension) && ["ass", "ssa"].includes(outExt)) {
    // ASS/SSA → ASS/SSA
  ...
  } else if (["ass", "ssa"].includes(inputExtension) && ["srt", "vtt"].includes(outExt)) {
    // ASS/SSA → SRT/VTT
    const { events } = parsedSubtitle;
    const format = outExt === "vtt" ? "WebVTT" : "SRT";
    const cues = events.map((e: any) => ({
      type: "cue" as const,
      data: {
        start: assTimeToMs(e.data.start),
        end: assTimeToMs(e.data.end),
        text: (e.data.translatedText || e.data.text).replace(/\\N/g, "\n"),
      },
    }));
    newSubtitle = stringifySync(cues, { format });
  } else if (["srt", "vtt"].includes(inputExtension) && ["ass", "ssa"].includes(outExt)) {
    // SRT/VTT → ASS/SSA
    const cues = parsedSubtitle
      .filter((x: any) => x.type === "cue")
      .map((x: any) => ({
        text: (x.data.translatedText || x.data.text).replace(/\n/g, "\\N"),
        start: msToAssTime(x.data.start),
        end: msToAssTime(x.data.end),
      }));
    newSubtitle = buildAssDocument(cues);
  } else {
    throw new Error(`Unsupported format conversion: ${inputExtension} → ${outExt}`);
  }
...
export async function translateFile(opts: TranslateFileOptions): Promise<void> {
  ...
  const outputExt = opts.outputFormat || ext;
  ...
  for (const block of chunks) {
    ...
    // Partial save
    try {
      saveTranslated(opts.outputPath, parsed, ext, outputExt);
    } catch {}
  }
