...
const ACCORDION_VARIANT_CLS: Record<AccordionVariant, { root: string; trigger: string; panel: string }> = {
  card: {
    root: "rounded-xl border border-[var(--border)] bg-[var(--surface)]",
    trigger: "px-4 py-3",
  ...
  return (
    <div className={`${styles.root} ${className}`}>
      <button
        ...
        className={`flex min-h-[44px] w-full items-center justify-between gap-3 text-[13px] font-medium text-[var(--text)] leading-6 ${styles.trigger}`}
      >
        <span>{title}</span>
