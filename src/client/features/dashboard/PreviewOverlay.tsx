...
export function PreviewOverlay({ isMobile, jobId, previewSearch, setPreviewSearch, onClose }: PreviewOverlayProps) {
  ...
  return (
    <ModalShell
      ...
      panelClassName={`mx-auto flex h-full w-full flex-col overflow-hidden border border-[var(--border)] bg-[var(--surface)] ${isMobile ? "rounded-none" : "max-w-6xl rounded-3xl md:max-h-[90vh]"}`}
    >
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="flex items-center justify-between gap-4 border-b border-[var(--border)] px-5 py-4">
          ...
          <input
            ...
            className="w-40 rounded-xl border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2 text-xs text-[var(--text)] focus:outline-none focus:border-[var(--accent)] md:w-56"
          />
          <button onClick={onClose} aria-label={t("common.close")} className="text-lg text-[var(--text-2)] hover:text-[var(--text)]">×</button>
        </div>
          <div className="border-b border-[var(--border)] px-4 py-2">
            <div className="flex flex-wrap gap-2 text-xs">
              <button onClick={() => setShowOnlyChanged((v) => !v)} className={`rounded-lg px-2 py-1 ${showOnlyChanged ? "bg-[var(--accent)] text-[var(--on-accent)]" : "bg-[var(--surface-2)] text-[var(--text-2)]"}`}>{t("dashboard.preview.showOnlyChanged")}</button>
              <button onClick={() => setShowOnlyIssues((v) => !v)} className={`rounded-lg px-2 py-1 ${showOnlyIssues ? "bg-[var(--yellow)] text-[var(--bg)]" : "bg-[var(--surface-2)] text-[var(--text-2)]"}`}>{t("dashboard.preview.showOnlyIssues")}</button>
              <button onClick={jumpToNextIssue} className="rounded-lg bg-[var(--surface-2)] px-2 py-1 text-[var(--text-2)]">{t("dashboard.preview.nextIssue")}</button>
            </div>
          </div>
          <div ref={listRef} className="flex-1 overflow-y-auto">
            ...
            {previewData?.analysis && (
              <div className="mx-4 mt-4 rounded-2xl border border-[var(--border)] bg-[var(--surface-2)] p-4">
                <div className="mb-2 text-xs uppercase tracking-wide text-[var(--text-3)]">Context / Plot Summary / Glossary</div>
                <pre className="whitespace-pre-wrap text-xs text-[var(--text)] leading-relaxed">{previewData.analysis}</pre>
                <div className="mt-3 flex flex-wrap gap-2 text-xs">
                  <button onClick={() => handleCopy(previewData.analysis || "", t("dashboard.preview.copiedContext"))} className="rounded-lg bg-[var(--surface-2)] px-2 py-1 text-[var(--text)]">{t("dashboard.preview.copyContext")}</button>
                  <button onClick={() => handleCopy(plot || "", t("dashboard.preview.copiedPlot"))} className="rounded-lg bg-[var(--surface-2)] px-2 py-1 text-[var(--text)]">{t("dashboard.preview.copyPlot")}</button>
                  <button onClick={() => handleCopy(glossary || "", t("dashboard.preview.copiedGlossary"))} className="rounded-lg bg-[var(--surface-2)] px-2 py-1 text-[var(--text)]">{t("dashboard.preview.copyGlossary")}</button>
                </div>
              </div>
            )}
            {previewData && filteredLines.length === 0 && !previewQuery.isLoading && (
              <div className="py-10 text-center text-sm text-[var(--text-3)]">{t("dashboard.preview.noLines")}</div>
            )}
          ...
          {previewData && filteredLines.length > 0 && (
            <div className="border-t border-[var(--border)] px-5 py-3 flex flex-wrap items-center justify-end gap-2">
              <button
                onClick={() => {
                ...
                className="rounded-xl border border-[var(--border)] px-3 py-2 text-xs text-[var(--text-2)] hover:text-[var(--text)]"
              >
...
function TranslatedEditor({ line, editCtx }: { line: PreviewLine; editCtx: EditCtx }) {
  ...
  return (
    <textarea
      ...
      className={`w-full resize-y rounded border bg-[var(--surface)] px-2 py-1 text-xs text-[var(--text)] focus:outline-none focus:border-[var(--accent)] ${
...
function PreviewDesktopTable({
  ...
  return (
    <table className="w-full text-sm">
      <thead className="sticky top-0 bg-[var(--surface)] text-[var(--text-3)] text-[10px] uppercase">
        <tr>
...
function PreviewDesktopVirtual({
  ...
  return (
    <div className="text-sm">
      <div
        className="sticky top-0 z-10 grid bg-[var(--surface)] text-[var(--text-3)] text-[10px] uppercase"
  ...
  if (table) {
    return (
      <tr id={`preview-line-${line.index}`} className={issue ? "bg-[var(--yellow-dim)]" : ""}>
        <td className="px-3 py-1.5 text-[var(--text-3)] text-[10px] align-top">{line.index}</td>
        <td className="px-3 py-1.5 text-[10px] text-[var(--text-3)] font-mono align-top whitespace-nowrap">{formatTimecode(line.start)}</td>
        <td className="px-3 py-1.5 text-[var(--text-2)] align-top text-xs">{line.original}</td>
        <td className="px-3 py-1.5 align-top text-xs"><TranslatedEditor line={line} editCtx={editCtx} /></td>
        <td className="px-3 py-1.5 align-top">{issue && <span className="text-[var(--yellow)] text-[10px]">⚠</span>}</td>
      </tr>
    );
  }

  return (
    <div id={`preview-line-${line.index}`} className={`rounded-2xl border p-3 ${issue ? "border-[var(--yellow-border)] bg-[var(--yellow-dim)]" : "border-[var(--border)] bg-[var(--surface-2)]"}`}>
      <div className="mb-2 flex items-center justify-between text-[11px] text-[var(--text-3)]">
...
function PreviewLineGridRow({
  ...
  return (
    ...
    >
      <div className="px-3 py-1.5 text-[var(--text-3)] text-[10px]">{line.index}</div>
      <div className="px-3 py-1.5 text-[10px] text-[var(--text-3)] font-mono whitespace-nowrap">{formatTimecode(line.start)}</div>
      <div className="px-3 py-1.5 text-[var(--text-2)] text-xs">{line.original}</div>
      <div className="px-3 py-1.5 text-xs"><TranslatedEditor line={line} editCtx={editCtx} /></div>
      <div className="px-3 py-1.5">{issue && <span className="text-[var(--yellow)] text-[10px]">⚠</span>}</div>
    </div>
  );
}
