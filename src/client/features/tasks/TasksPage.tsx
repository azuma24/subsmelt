    ...
    <div className="flex min-h-full flex-col">
      ...
      <div className="flex-1 space-y-4 p-3.5 md:p-[18px]">
        ...
        <SelectionBar
          ...
          isMobile={isMobile}
        >
          <button onClick={() => applyBulk({ enabled: 1 })} disabled={bulkCounts.disabled === 0} className="rounded-lg bg-[var(--green)] px-3 py-2 text-xs font-semibold text-[var(--on-accent)] disabled:opacity-40">{t("translation_languages.bulk.enable")}</button>
      ...
      {editing && (
        ...
        >
          <div className="mt-4 space-y-4">
                    ...
                    {OUTPUT_FORMATS.map((format) => {
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-full px-3 py-1.5 text-xs font-semibold uppercase tracking-wide ${active ? "bg-[var(--accent)] text-[var(--on-accent)]" : "bg-[var(--surface-2)] text-[var(--text-2)] hover:bg-[var(--surface-3)] hover:text-[var(--text)]"}`}
                        >
                          {format}
                        </button>
                      );
                ...
                {hasMismatch && (
                  <div className="rounded-lg border border-[var(--yellow-border)] bg-[var(--yellow-dim)] p-3 text-xs text-[var(--yellow)]">
                    ...
                    <div>
                      <button onClick={() => setEditing({ ...editing, output_pattern: applyOutputFormat(editing.output_pattern, selectedOutputFormat) })} className="mt-2 rounded-lg bg-[var(--yellow)] px-2 py-1 font-medium text-[var(--on-accent)]">
          ...
          <div className={`mt-6 flex gap-3 ${isMobile ? "sticky bottom-0 bg-[var(--surface)] pt-4" : "justify-end"}`}>
            ...
            <button onClick={handleSave} disabled={!canSave} className="flex-1 rounded-lg bg-[var(--accent)] px-4 py-2.5 text-[13px] font-medium text-[var(--on-accent)] disabled:opacity-50 md:flex-none">{isNew ? t("translation_languages.create") : t("common.save")}</button>
          </div>
        </ModalShell>
      )}
    </div>
