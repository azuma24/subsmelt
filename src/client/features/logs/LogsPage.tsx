...
export function LogsPage({ isMobile }: { isMobile: boolean }) {
  ...
  // The Whisper backend keeps its own log on its own host — a separate source,
  // not another category of app log, so it gets a source switch rather than
  // being merged into the app's store.
  const [source, setSource] = useState<LogSource>("app");
  const logsQuery = useLogsQuery(level, category, jobIdFilter);
  const logs = logsQuery.data || [];
  const backendLogsQuery = useTranscriptionLogsQuery(source === "backend", follow);
    ...
    <div className="flex h-full flex-col">
      ...
      <div className={`sticky top-0 z-30 shrink-0 border-b border-[var(--border)] bg-[var(--surface)] px-3.5 py-2 md:px-[18px] ${isMobile ? "space-y-2" : ""}`}>
        ...
        <div className="flex min-h-[42px] items-center gap-2.5">
          ...
          />
          {source === "app" && jobIdFilter && <span className="text-[11px] text-[var(--accent)]">{t("logs.filteredByJob", { id: jobIdFilter })}</span>}
