...
.PARAMETER Token
    ...
    Leave blank and pass -GenerateToken to have one minted for you.

.PARAMETER GenerateToken
    Mint a strong token instead of supplying one. It is printed at the end of
    the install for pasting into SubSmelt -> Settings -> Speech to Text ->
    Backend token. Ignored when -Token is given.
...
# -GenerateToken delegates to the launcher so the service and the control window
# mint keys the same way (app/token_gen.py), rather than this script inventing a
# second, weaker notion of "random enough".
$minted = $false
if ($Token -eq "" -and $GenerateToken) {
    $generated = & $ExePath --generate-token |
        Select-String -Pattern '^\s{4}(\S+)$' |
    ...
    if ($generated) {
        ...
        Write-Warning "Could not generate a token via $ExePath --generate-token; continuing without one."
    }
}
if ($Token -ne "") {
    ...
    if ($minted) {
        Write-Host ""
        Write-Host "Backend API key: $Token" -ForegroundColor Green
        Write-Host "Paste it into SubSmelt -> Settings -> Speech to Text -> Backend token."
        Write-Host ""
    }
} else {
    Write-Warning "No token set: the backend binds 0.0.0.0 and will accept requests from any host on this network."
    Write-Warning "Re-run with -GenerateToken to have one created for you."
}
